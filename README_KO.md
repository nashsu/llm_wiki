...
- **Codexian Memory** — Codex가 사용자의 profile, decision, workflow, session을 기억 문맥으로 읽고 다음 작업에 반영할 수 있게 합니다.
...
### 2.5 Codexian Memory / 나를 기억하는 LLM Wiki

Codexian Memory 템플릿은 LLM Wiki를 “Codex가 Kevin을 기억하는 개인 Wiki”로 사용하기 위한 구조를 추가합니다. 기존 Raw Sources → Wiki → Schema 원칙은 유지하고, Codex가 매번 다시 설명을 요구하지 않도록 장기 기억과 최근 작업 맥락을 분리합니다.

```text
wiki/
├── profile/      # 사용자 목표, 선호, 판단 기준, 응답 스타일
├── decisions/    # 확정된 선택과 이유, 되돌릴 조건
├── workflows/    # 반복 작업 절차와 세션 시작 규칙
├── sessions/     # 최근 작업 요약과 handoff 맥락
└── synthesis/
    └── codex-boot-context.md  # Codex 시작용 압축 기억
```
...
│   ├── comparisons/        # 비교 분석
│   ├── profile/            # Codexian Memory: 사용자 profile
│   ├── decisions/          # Codexian Memory: 확정 결정
│   ├── workflows/          # Codexian Memory: 반복 절차
│   └── sessions/           # Codexian Memory: 최근 작업 맥락
