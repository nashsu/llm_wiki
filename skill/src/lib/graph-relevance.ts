...
function extractFrontmatter(content: string): { title: string; type: string; sources: string[] } {
  ...
  return { title, type: typeMatch ? typeMatch[1].trim().toLowerCase() : "other", sources }
}
...
export async function buildRetrievalGraph(
  ...
  if (cachedGraph !== null && cachedGraph.dataVersion === dataVersion) return cachedGraph
  ...
  const rawNodes: Array<{
    ...
    sources: string[]; rawLinks: string[]; fileName: string
  }> = []
  ...
  for (const file of mdFiles) {
    ...
    rawNodes.push({
      ...
      rawLinks: extractWikilinks(content), fileName: file.name,
    })
  }
...
export function calculateRelevance(
  nodeA: RetrievalNode, nodeB: RetrievalNode, graph: RetrievalGraph,
): number {
  ...
  for (const src of nodeB.sources) { if (sourcesA.has(src)) sharedSourceCount += 1 }
...
export function getRelatedNodes(
  nodeId: string, graph: RetrievalGraph, limit: number = 5,
