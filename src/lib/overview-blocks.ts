...
export function buildOverviewPrematchPrompt(sourceContent: string, chunk: string): string {
  return [
    ...
    "For each paragraph, determine whether the source adds NEW information that",
    "would meaningfully change or extend it. Do NOT match a paragraph if it",
    "already covers the topic adequately and the source only restates the same",
    "facts — re-ingesting the same topic does not require an update.",
    "",
    chunk,
    "",
