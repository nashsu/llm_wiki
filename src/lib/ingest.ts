...
export function buildAnalysisPrompt(purpose: string, index: string, sourceContent: string = ""): string {
  return [
    ...
    "",
    "## Codexian Memory Classification",
    "If the source is about Kevin, Codex sessions, agent workflows, preferences, decisions, or recent work continuity, classify useful memory into:",
    "- profile: durable user preferences, operating model, judgment criteria, or response style",
    "- decision: confirmed choices and rationale that should guide future work",
    "- workflow: repeatable procedures Codex should follow",
    "- session: recent work context, open loops, and handoff notes",
    "- synthesis: compact boot context that helps Codex start with the right memory",
    "Profile changes must be treated as candidates requiring human review unless the source explicitly confirms they are already reviewed.",
    "",
...
export function buildGenerationPrompt(schema: string, purpose: string, index: string, sourceFileName: string, overview?: string, sourceContent: string = ""): string {
  ...
  return [
    ...
    "4. If the analysis identified Codexian Memory, create decision pages in wiki/decisions/, workflow pages in wiki/workflows/, session pages in wiki/sessions/, and synthesis updates in wiki/synthesis/ as appropriate",
    "5. Do NOT silently overwrite durable profile pages. For profile changes, emit a REVIEW block unless the source explicitly says the profile memory is already reviewed.",
    ...
    "  • type     — one of: source | entity | concept | comparison | query | synthesis | decision | workflow | session | profile",
    ...
    "- confirm: profile memory that should be reviewed before becoming active",
    "",
    ...
    "- confirm: OPTIONS: Create Page | Skip",
    "",
