...
describe("chunkOverviewBySections", () => {
  ...
  it("returns single chunk when overview is small", () => {
    ...
    expect(chunks[0]).toContain("操作系统是管理硬件资源的软件。")
    expect(chunks[0]).toContain("[2]")
    expect(chunks[0]).toContain("计算机网络是互联互通的系统。")
  })
  ...
  it("handles overview with no ## headings (single chunk)", () => {
    ...
    expect(chunks).toHaveLength(1)
    expect(chunks[0]).toContain("[1]")
    expect(chunks[0]).toContain("Just some prose without headings.")
  })
})

describe("parseOverviewPrematchOutput", () => {
  it("parses bracket paragraph numbers", () => {
    expect(parseOverviewPrematchOutput("[2, 5, 12]")).toEqual([2, 5, 12])
  })
  ...
  it("handles extra whitespace", () => {
    expect(parseOverviewPrematchOutput("[ 2 ,  5 ]")).toEqual([2, 5])
  })
})
...
describe("buildOverviewPrematchPrompt", () => {
  it("includes source content and chunk", () => {
    ...
    expect(prompt).toContain("paragraph numbers")
  })
})
...
describe("assembleReducedOverview", () => {
  ...
  it("assembles only matched paragraphs", () => {
    const overview = [
      "# Overview", "",
      "## 操作系统", "OS content", "",
      "## 网络", "Network content", "",
    ...
    // Paragraphs: [1] preamble, [2] 操作系统, [3] 网络, [4] 数据库
    const result = assembleReducedOverview(overview, [2, 4])
...
describe("runOverviewPrematchParallel", () => {
  ...
  it("collects matched paragraph numbers from all chunks", async () => {
    mockStreamChat
      .mockImplementationOnce(mockStreamResponse("[1, 3]"))
      .mockImplementationOnce(mockStreamResponse("[5]"))
