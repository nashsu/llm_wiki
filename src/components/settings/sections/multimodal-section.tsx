...
export function MultimodalSection({ draft, setDraft }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {draft.multimodalEnabled && (
        <>
          {/* Markdown image localizer toggle (v0.6.6). Downloads remote
              images referenced in .md sources to wiki/media/<slug>/,
              rewrites URLs to local relative paths, and fills alt text
              via the VLM. Independent of the captioning pipeline above
              (which handles PDF/DOCX/PPTX extracted images). */}
          <div className="flex items-center justify-between gap-3 rounded-md border p-3">
            <div className="min-w-0 flex-1">
              <div className="text-sm font-medium">
                {t("settings.sections.multimodal.localizeLabel", "Download remote images")}
              </div>
              <div className="text-xs text-muted-foreground">
                {t(
                  "settings.sections.multimodal.localizeHint",
                  "When importing .md files, download remote images (e.g. WeChat article images) to the local wiki/media/ folder and rewrite URLs. Alt text is generated by the vision model for accessibility. Off: remote image links are kept as-is and may break if the source goes offline.",
                )}
              </div>
            </div>
            <button
              type="button"
              onClick={() => setDraft("multimodalLocalizeImages", !draft.multimodalLocalizeImages)}
              role="switch"
              aria-checked={draft.multimodalLocalizeImages}
              aria-label={t("settings.sections.multimodal.localizeLabel", "Download remote images")}
              className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${
                draft.multimodalLocalizeImages ? "bg-primary" : "bg-muted"
              }`}
            >
              <span
                className={`inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform ${
                  draft.multimodalLocalizeImages ? "translate-x-4.5" : "translate-x-0.5"
                }`}
              />
            </button>
          </div>
