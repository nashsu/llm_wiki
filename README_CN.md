...
LLM Wiki 是一个跨平台桌面应用，能将你的文档自动转化为可枚举、可导航的**概念索引**。它不是"编译一次、永不重推"的答案缓存。

这里存在两种本质不同的知识合成：

- **资料内在合成** —— 资料本身断言的张力、对比与规律，可在摄入时预先编译，Wiki 在摄入阶段捕获这部分。
- **查询内在合成** —— 针对你具体场景的权衡与阐释，无法预先编译——摄入时无法预知查询意图。

Wiki 的职责是为聊天层提供**知识基底与空白感知**：你可以问"是否存在名为 X 的页面"并得到确定答案——块存储无法做到这一点。聊天层在查询时完成合成，Wiki 让这个合成过程可审计、可导航、可感知空白。
