...
- **多格式文档解析** — 支持 PDF、Office 文档、EPUB/MOBI、Org mode、图片、音视频、网页剪藏和批量 URL 导入，并提供内置、云端或本地 MinerU PDF 处理。针对公开 YouTube 单视频 URL，应用会以实验性的尽力而为方式提取可用的原语种字幕文本；若没有合格字幕或专用提取失败，则回退为不含字幕文稿的普通网页导入
