// Server port of src/lib/sources-merge.ts for the server-driven
// ingest pipeline (issue #14). Behavior is byte-identical to the
// client module; only types were stripped.
/**
 ...
 */
export function parseFrontmatterArray(content, fieldName) {
 ...
 * shape.
 */
export function writeFrontmatterArray(content, fieldName, values) {
  const fmMatch = content.match(/^(---\n)([\s\S]*?)(\n---)/)
 ...
 */
export function mergeArrayFieldsIntoContent(newContent, existingContent, fields) {
 ...
 * sources list back).
 */
export function writeSources(content, sources) {
 ...
 */
export function mergeSourcesLists(existing, incoming) {
 ...
 */
export function mergeSourcesIntoContent(newContent, existingContent) {
