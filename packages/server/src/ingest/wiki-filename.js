 ...
 *
 * Ported verbatim from src/lib/wiki-filename.ts for the server-driven ingest
 * cutover (issue #14). Pure logic — no browser/Tauri APIs involved.
 *
...
export function makeQuerySlug(title) {
  const slug = title
    .normalize("NFKC")
    .trim()
    .replace(/\s+/g, "-")
