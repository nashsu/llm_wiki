// Server port of src/lib/page-merge.ts for the server-driven ingest
// pipeline (issue #14). Behavior is byte-identical to the client
// module; only types were stripped. The LLM merger is injected as a
// callback (the orchestrator supplies it); this module stays pure
// logic + seam.
/**
...
/**
 * @typedef {object} MergePageOptions
 * @property {string} sourceFileName Source filename being ingested —
 *   passed through to the merger and used in fallback log messages.
 * @property {string} pagePath Wiki-relative page path (e.g.
 *   wiki/entities/foo.md). Used only for log messages today; the
 *   file write itself is the caller's responsibility.
 * @property {AbortSignal} [signal] Abort signal forwarded to the merger.
 * @property {(existingContent: string) => Promise<void>} [backup]
 *   Best-effort hook to back up the existing content before any
 *   fallback write. Errors are swallowed — backup must never block
 *   the merge.
 * @property {() => string} [today] Date provider for the `updated`
 *   field. Injectable for deterministic tests. Defaults to today's
 *   UTC date.
 * @property {boolean} [replaceExistingBody] Replace the old body
 *   after a corrected source is regenerated, while retaining union
 *   fields, locked metadata, and a recovery backup.
 */

export async function mergePageContent(newContent, existingContent, merger, opts) {
