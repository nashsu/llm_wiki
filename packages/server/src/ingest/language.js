// Language utilities for server-driven ingest (ported verbatim from the
// browser client for issue #14's P0 "server-driven ingest" cutover).
//
// Ported from:
//   - src/lib/detect-language.ts      → detectLanguage (+ script helpers)
//   - src/lib/language-metadata.ts    → getLanguageMetadata / getLanguagePromptName /
//                                       getTextDirection / getHtmlLang / sameScriptFamily
//   - src/lib/output-language.ts      → getOutputLanguage / buildLanguageDirective /
//                                       buildLanguageReminder
//
// Behavioral note (only port deviation): the client's getOutputLanguage /
// buildLanguageDirective / buildLanguageReminder read the outputLanguage
// setting from the Zustand wiki store (useWikiStore.getState().outputLanguage).
// On the server there is no store, so the setting value is passed as the FIRST
// PARAMETER instead. All regexes, constants, magic numbers, and algorithms are
// byte-identical to the client source.
...
export function getLanguagePromptName(language) {
...
export function sameScriptFamily(a, b) {
 ...
 *
 * PORT DEVIATION: the client reads useWikiStore.getState().outputLanguage;
 * the server version takes the setting value as a parameter instead.
 */
export function getOutputLanguage(outputLanguage, fallbackText = "") {
  const configured = outputLanguage
 ...
 * the server version takes the setting value as a parameter instead.
 */
export function buildLanguageDirective(outputLanguage, fallbackText = "") {
  const lang = getOutputLanguage(outputLanguage, fallbackText)
 ...
 * the server version takes the setting value as a parameter instead.
 */
export function buildLanguageReminder(outputLanguage, fallbackText = "") {
  const lang = getOutputLanguage(outputLanguage, fallbackText)
