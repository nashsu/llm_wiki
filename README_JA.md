...
- **マルチフォーマット文書解析** — PDF、Office 文書、EPUB/MOBI、Org mode、画像、メディア、Web クリップ、URL の一括インポートに対応し、内蔵・クラウド・ローカル MinerU による PDF 処理を選択可能。公開されている YouTube の単一動画 URL では、利用可能な元言語の字幕トランスクリプトを実験的かつベストエフォートで抽出し、対象字幕がない場合や専用抽出に失敗した場合は、トランスクリプトなしの通常の Web ページインポートにフォールバックします
